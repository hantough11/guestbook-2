...
(function(){
	...
	window.lgKb = {
		...
		onKeyUp :				//	user's keyup event handler
			function (event)
			{
			    $('#lay_editor_layer .text .contents .text_input #input_text_input').keyup();
			},
...
//						return;		//	Virtual Keyboard works well on LG Smart TV
				...
				if(!lgKb.vKeyboard)
				{
					...
					console.log("lgkb path : " + lgKb.vKbJsRootPath);
				...
				if(keyId.search(/wkk_key/)>=0)
				{
					switch(keyId)
					{
					    ...
					    case 'wkk_key_hide':
					        ...
					        if (currentFocusLeftMenu == 6)
					        {
					            currentFocusTab = "textarea_items";
					            $('#lay_editor_layer .text .contents .text_font').addClass("focus");
					        }
					        else if (currentFocusLeftMenu == 2)
					        {
					            currentFocusTab = "bottom";
					            $("#lay_editor_layer .publish .contents .bottom_btn .apply").addClass("focus");
					        }
							lgKb.focusOut();
							break;
						default : 
