<?php	
	...
	if($file_list) {
		...
		// 지울 이미지 번호 읽기		
    $deleteNumner = count($file_list_json['background_images_list']) - $_GET['selectedMyImagesNo'] + 1;
    echo $deleteNumner;
		echo "<br><br>";	
		
		// 이미지 파일 삭제
		$filename = $file_list_json["background_images_list"][$deleteNumner-1]["filename"];
		$object_image_file = "gs://popon-tv/".$user_email."/background_images/".$filename;
	...
	echo("<script language='javascript'>parent.window.php_complete_background_images_delete();</script>");
